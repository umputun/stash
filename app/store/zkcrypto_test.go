...
func TestNewZKCrypto_KeyValidation(t *testing.T) {
	tests := []struct {
		name      string
		key       []byte
		wantError bool
	}{
		{"valid key 16 bytes", []byte("1234567890123456"), false},
		{"valid key 32 bytes", []byte("12345678901234567890123456789012"), false},
